...
def generate_data(n_points, centroid_num, n_features):
    ...
    pyplot.show()
    # 保存数据
    np.savetxt('./results/usv_position', data)
    # 读取数据
...
def sampling_and_clustering(data, n_samples, centroid_num):
    ...
    centroids, _, _ = kmedoids.pam(subset, centroid_num)
...
if __name__ == '__main__':
    n_points = 50  # 生成的数据的个数
    subset_size = 6  # phase 1中采样后子集的大小
    subset_num = 6  # phase 1中采样的子集的数量
    centroid_num = 6  # 簇中心的数量
    n_features = 2  # 数据维度
    main()
